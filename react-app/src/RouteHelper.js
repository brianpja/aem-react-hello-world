 ...
 * @param {string} [routeKey=:path]             - name of the property stored in the model that contains the url to be used as a route
 * @param {string} [extension=html]             - extension used to identify a route amongst the tree of resource URLs
 * @returns {CompositeRoute}
 */
export const withRoute = (WrappedComponent, routeKey, extension) => {
    return class CompositeRoute extends Component {
        render() {
            routeKey = routeKey || Constants.PATH_PROP;

            let routePath = this.props.cqModel && this.props.cqModel[routeKey];
